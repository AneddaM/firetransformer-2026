...
def _resolve_column(columns, aliases, required=True):
    ...
    # Conservative contains fallback for descriptive names such as Gas Resistance [Ohm].
    for a in aliases:
        na = _norm(a)
        if len(na) < 4:
            continue
        for nc, original in normalized.items():
            if na in nc or nc in na:
                return original
    if required:
        raise KeyError(f"Could not resolve any of aliases {aliases}. Available columns: {list(columns)}")
    return None
...
def resolve_schema(df: pd.DataFrame, schema_cfg: dict):
    ...
    for canonical, aliases in schema_cfg["feature_aliases"].items():
        mapping[canonical] = _resolve_column(df.columns, [canonical] + list(aliases))
    mapping["fire"] = _resolve_column(df.columns, schema_cfg.get("fire_aliases", ["fire", "label"]))
    mapping["timestamp"] = _resolve_column(
        df.columns, schema_cfg.get("timestamp_aliases", ["timestamp", "time"]), required=False
    )
    return mapping
...
class DatasetCatalog:
    def __init__(self, root, schema_cfg):
        ...
        for p in list_csvs(self.root):
            ...
            fire = normalize_fire_label(df.loc[valid, mapping["fire"]].reset_index(drop=True))
            ts_series = None if mapping["timestamp"] is None else df.loc[valid, mapping["timestamp"]].reset_index(drop=True)
            ts = parse_timestamps(ts_series, len(canonical))
            if len(canonical) == 0:
                continue
            self.acquisitions.append(Acquisition(p, infer_node(p), canonical, fire, ts))
...
def split_outer_inner(catalog: DatasetCatalog, heldout_node: str, val_fraction_files=0.25, split_seed=1337):
    ...
    for node in sorted({a.node for a in dev}):
        files = [a for a in dev if a.node == node]
        idx = np.arange(len(files))
        rng.shuffle(idx)
        n_val = max(1, int(round(len(files) * val_fraction_files))) if len(files) > 1 else 0
        val_idx = set(idx[:n_val].tolist())
        for i, a in enumerate(files):
            (val if i in val_idx else train).append(a)
    if not train or not val:
        raise ValueError("Need at least two acquisition files among development nodes for grouped train/validation split")
...
def fit_feature_scaler(train_acqs, rolling_window=5):
    ...
    scaler = MinMaxScaler().fit(pd.concat(frames, ignore_index=True).to_numpy(dtype=np.float64))
    return scaler, feature_names


def make_windows_one(acq: Acquisition, scaler: MinMaxScaler, rolling_window=5, window=60, horizon=15):
    f = enrich(acq.raw, rolling_window).to_numpy(dtype=np.float64)
    x = scaler.transform(f).astype(np.float32)
    yraw = acq.fire
    ts = acq.timestamps
    X, y, end_ts, onset_ts, file_ids = [], [], [], [], []
    last_t = len(x) - horizon - 1
    for t in range(window - 1, last_t + 1):
        future = yraw[t + 1 : t + horizon + 1]
        pos = np.flatnonzero(future == 1)
        label = int(len(pos) > 0)
        X.append(x[t - window + 1 : t + 1])
        y.append(label)
        end_ts.append(ts[t])
        onset_ts.append(ts[t + 1 + pos[0]] if label else np.nan)
        file_ids.append(str(acq.path))
    if not X:
        nfeat = x.shape[1]
        return WindowBundle(
            np.empty((0, window, nfeat), np.float32), np.empty((0,), np.int64),
            np.empty((0,), np.float64), np.empty((0,), np.float64), np.empty((0,), object)
        )
    return WindowBundle(
        np.stack(X), np.asarray(y, np.int64), np.asarray(end_ts, np.float64),
        np.asarray(onset_ts, np.float64), np.asarray(file_ids, object)
    )
...
def concat_bundles(bundles):
    ...
    if not good:
        raise ValueError("No windows generated. Check window/horizon length and input files.")
    return WindowBundle(
...
def build_fold(catalog, heldout_node, rolling_window=5, window=60, horizon=15,
               val_fraction_files=0.25, split_seed=1337):
    train_a, val_a, test_a = split_outer_inner(catalog, heldout_node, val_fraction_files, split_seed)
    scaler, feature_names = fit_feature_scaler(train_a, rolling_window)
    make = lambda items: concat_bundles([
        make_windows_one(a, scaler, rolling_window, window, horizon) for a in items
    ])
    train, val, test = make(train_a), make(val_a), make(test_a)
    return train, val, test, scaler, feature_names, train_a, val_a, test_a


def to_loader(bundle: WindowBundle, batch_size=256, shuffle=False, num_workers=0):
    ds = TensorDataset(torch.from_numpy(bundle.X), torch.from_numpy(bundle.y))
