...
class FireTransformer(nn.Module):
    def __init__(self, n_features=20, d_model=64, n_heads=4, n_layers=3, d_ff=128,
                 dropout=0.1, gas_feature_index=3):
        super().__init__()
        ...
        if gas_feature_index is not None and 0 <= gas_feature_index < n_features:
            with torch.no_grad():
                self.input_proj.weight[:, gas_feature_index].mul_(2.0)
    ...
    def forward(self, x, return_attention=False):
        ...
        h = self.encoder(h)
        h_cls = h[:, 0]
        pooled, a = self.pool(h[:, 1:])
        logits = self.classifier(torch.cat([h_cls, pooled], dim=-1)).squeeze(-1)
        if return_attention:
            return logits, a
        return logits
...
def build_model(model_cfg, n_features=20, gas_feature_index=3):
    if model_cfg["type"] == "transformer":
        return FireTransformer(
            ...
            gas_feature_index=gas_feature_index,
        )
    if model_cfg["type"] == "bilstm":
